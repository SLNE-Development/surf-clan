...
import dev.slne.clan.core.member.CoreClanMemberService
...
class ClanMemberServiceImpl : CoreClanMemberService {
    ...
    override suspend fun addMember(
        ...
        val result = ClanMemberRepository.createMember(clanID, player, role, invitedBy)
        if (result is ClanMemberAddResult.Success) {
            ClanServiceImpl.get().invalidateCachedClanByID(clanID)
        }
    ...
    override suspend fun removeMember(clanID: ULong, player: UUID): Boolean {
        val result = ClanMemberRepository.deleteMember(clanID, player)
        if (result) {
            ClanServiceImpl.get().invalidateCachedClanByID(clanID)
        }
    ...
    override suspend fun changeRole(member: ClanMemberImpl, role: ClanMemberRole): Boolean {
        val changed = ClanMemberRepository.changeRole(member.id, role)
